...
import marketbot.channels.matrix as matrix_module
from marketbot.bus.events import OutboundMessage
from marketbot.bus.queue import MessageBus
from marketbot.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from marketbot.config.schema import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("marketbot.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "marketbot.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("marketbot.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "marketbot.channels.matrix.asyncio.create_task", _fake_create_task
    )
