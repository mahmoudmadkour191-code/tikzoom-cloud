...
from vadana.connect import (parse_recording_url, ConnectClient, Recording,
                            is_valid_recording, resolve_room_recording)
...
async def _archive(path, thumb=None, uid=None, link=None):
    ...
    # big class videos (100MB+) need far longer than aiogram's default request
    # timeout to reach Telegram; without this the job dies at the very last step
    ...
    msg = await bot.send_document(config.STORAGE_CHANNEL, FSInputFile(path), caption=caption,
                                  parse_mode="HTML", thumbnail=th, request_timeout=1800)
...
async def _run_job(m, rec, mode, uid):
    ...
    try:
        # an uppercase id is a meeting-room link (IAU summer term), not a recording;
        # swap in the room's newest recording so the normal pipeline (and its cache,
        # keyed by the real rec_id) just works. Lowercase links skip this entirely.
        if re.search(r"[A-Z]", rec.rec_id):
            rid = await asyncio.to_thread(resolve_room_recording, rec, config.IRAN_PROXY)
            if rid and rid != rec.rec_id:
                log.info("room link resolved: %s -> %s", rec.rec_id, rid)
                rec = Recording(host=rec.host, rec_id=rid, token=rec.token)
        if mode == "video":
...
async def do_video(m, rec, uid):
    ...
    try:
        ...
        stop.set(); await poller
        if result is None:
            if zf is None:      # cancelled before the local build even opened the package
                return
