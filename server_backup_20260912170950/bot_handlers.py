...
async def _handle_callback(client: TgClient, cb: dict) -> None:
    ...
    if data == "mcv_templates" or data.startswith("tpl_page_"):
        # قوالب بوتات جاهزة — قائمة صفحية لكل القوالب
        await ack()
        from .bot_templates import list_templates
        page = int(data.split("_")[-1]) if data.startswith("tpl_page_") else 0
        ...
        for i in range(0, len(page_items), 2):
            row = []
            for tid, tpl in page_items[i:i + 2]:
                row.append(Btn(f"{tpl['icon']} {tpl['name']}",
                               callback_data=f"tpl_{tid}", color="blue"))
            rows.append(row)
        nav = []
        if page > 0:
            nav.append(Btn("◀️ السابق", callback_data=f"tpl_page_{page - 1}", color="green"))
        nav.append(Btn(f"📄 {page + 1}/{total_pages} ({len(items)} قالب)",
                       callback_data=f"tpl_page_{page}"))
        if page < total_pages - 1:
            nav.append(Btn("التالي ▶️", callback_data=f"tpl_page_{page + 1}", color="green"))
        rows.append(nav)
        rows.append([Btn(t(lang, "btn_back"), callback_data="mcv_menu", color="blue")])
        await client.edit_message_text(
            chat_id, msg_id,
            "📦 <b>مكتبة بوتات GitHub الاحترافية</b>\n\n"
            f"<b>{len(items)}</b> بوت حقيقي مفتوح المصدر بأقوى المشاريع.\n"
            "اختار البوت وسيطلب منك: 🔑 توكن البوت + 👑 معرّف الأدمن (إلزامي)"
            " + متغيرات اختيارية لو احتاجها البوت.\n\n"
            "✨ <i>كل البوتات:</i>\n"
            "• مشاريع كبيرة متعددة الملفات من GitHub\n"
            "• تُثبّت مكتباتها تلقائياً في حاوية معزولة\n"
            "• تُشغّل long-polling بدون إعداد\n"
            "• مفحوصة ومختبرة داخل بيئة TikZoom",  # noqa: RUF001
