...
def test_new_style_flv_only_package(fake_client, make_zip, tmp_path):
    # 2026 summer-term packages ship raw FLVs with no XML manifests at all:
    # no shared-file list to read -> empty result, not a crash
    z = make_zip({"mainstream.flv": "x", "cameraVoip_0_3.flv": "x"})
    assert download_slides(fake_client, "rec", str(tmp_path), zf=z) == []


def test_truly_broken_package_still_raises(fake_client, make_zip, tmp_path):
    import pytest as _pytest
    z = make_zip({"random.txt": "x"})
